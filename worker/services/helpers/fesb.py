...
async def _call_fesb(host: str, port: int, endpoint: str, user: str, password: str, payload: dict,
                     method: str = 'POST', session: ClientSession | None = None) -> ClientResponse:
    """
    ...
    :param session: Активная http сессия. При отсутствии создается и завершается автоматически.
        ...
        async with asyncio.timeout(request_timeout):
            if not session:
                async with aiohttp.ClientSession() as new_session:
                    resp = await new_session.post(url, json=payload, auth=aiohttp.BasicAuth(user, password))
            else:
                resp = await session.post(url, json=payload, auth=aiohttp.BasicAuth(user, password))
...
async def get_message_error_text(exchange_id: str, request_id: str, session: aiohttp.ClientSession) -> str:
    """
    ...
    resp = await _call_fesb(HOST, PORT, ERROR_MESSAGE_ENDPOINT, USER, PASSWORD, payload, 'POST', session)
...
async def get_new_messages(start_date: datetime, end_date: datetime, limit: int | None = None,
    ...
    async with aiohttp.ClientSession() as http_session:
        resp = await _call_fesb(HOST, PORT, MESSAGES_LOG_ENDPOINT, USER, PASSWORD, payload, 'POST', http_session)
        resp_payload = await resp.json()
...
async def get_message_info(exchange_id: str) -> dict:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        resp = await _call_fesb(HOST, PORT, MESSAGES_LOG_ENDPOINT, USER, PASSWORD, payload, 'POST', session)
        resp_payload = await resp.json()
        msg_list = resp_payload.get('list')
        msg_count = resp_payload.get('count')
